...
import { readModelInfo } from '@/core/llama';
import { repoUrl } from '@/features/models/huggingface';
import useModelStore, { useInstalledModel } from '@/features/models/store';
...
import { formatBytes, formatParams } from '@/shared/utils/format';
...
const ModelDetail = ({ navigation, route }: ModelDetailScreenProps) => {
  ...
  const isSelected = useEffectiveModel().id === modelId;
  ...
  return (
    ...
    >
      ...
      <List.Section>
        ...
        <Row
          label="Size on disk"
          value={formatBytes(model.sizeBytes + (model.mmproj?.sizeBytes ?? 0))}
        />
        <Row label="File" value={model.filename} />
        <Row label="Reads images" value={model.mmprojPath ? 'Yes' : 'No'} />
        {model.mmproj && (
          <Row
