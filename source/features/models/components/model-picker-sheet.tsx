...
/**
 * Choosing which model answers, from wherever the question comes up.
 *
 * A sheet rather than a screen because picking a model is a detour, not a
 * destination — the conversation stays visible behind it, and it opens the
 * same way from the composer chip, the welcome card and the header button.
 */

// The sheet sizes itself to the list, but a long list must not swallow the
// conversation behind it — past this it scrolls instead of growing.
    ...
    >
      <BottomSheetView style={{ paddingBottom: insets.bottom + 12 }}>
        ...
        <List.Item
          ...
          onPress={() => go(() => navigation.navigate('ModelLibrary'))}
        />
        ...
        <List.Item
          ...
          right={props => <List.Icon {...props} icon="chevron-right" />}
          onPress={() =>
            go(() => navigation.navigate('GenerationSettings', {}))
          }
        />
      </BottomSheetView>
    </BottomSheetModal>
