...
export async function searchGgufRepos(
  ...
  const filterParams: Record<string, string> = {
    ...
    withCount: filters?.withCount ?? 'true', // include download and like counts,
    // sort: filters?.sort?.value ?? 'trending', // sort by trending by default
    // ...(filters?.language ? { language: filters.language.map(l => l.value).join(',') } : {}),
    ...(filters?.pipeline_tag ? { pipeline_tag: filters.pipeline_tag.value } : {}),
    ...(filters?.num_parameters ? { num_parameters: filters.num_parameters } : {}),
  };
